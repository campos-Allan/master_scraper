...
Canedo na pasta pycache')
    dica.insert(END,
                '\n\n4)Baixe o saldo e os relatórios de descarga rodoviária\n\
e ferroviária do TECIAP na pasta pycache')
...
15/11/2024, então se não tiver mudado,\n\
...
conforme estão até o dia 15/11/2024\n\
se houverem mudanças após esse dia pode ser que o\n\
programa não funcione mais')
    dica.insert(END, '\n\n9)O programa consegue ler PDF de múltiplos dias em todos\n\
arquivos possíveis,mas para facilitar o entedimento vale a pena ter\n\
o dia de cada arquivo no nome do arquivo,\n\
às vezes vem sem e é preciso editar manualmente')
    dica.insert(
...
def nova(function, polo):
    ...
    def copiar_rodo3():
        ...
        pyperclip.copy(text_gasoa5.get('1.0', END).strip()+'\t' +
                       text_s10_5.get('1.0', END).strip()+'\t'+text_s500_5.get('1.0', END).strip())
    ...
    def mov():
        ...
        movimentacao = Text(mov_janela, height=30, width=110)
    ...
    if function == 'excel':
        if polo == 'SINOP':
            ...
            if resultado_rv == 'Sem entradas':
                nova_janela.destroy()
                messagebox.showerror(
                    "Error", "Sem arquivos de Rio Verde - Planilha Basis(1)")
            else:
                ...
                text_gasoa1.configure(state="disabled")
                text_s500_1.configure(state="disabled")
                entradas_dtc = resultado_rv[2]['Unnamed: 3'][2:8][resultado_rv[2]
                                                                  ['Unnamed: 3'][2:8] != 0].to_list()
                saidas_dtc = resultado_rv[2]['Unnamed: 5'][2:8][resultado_rv[2]
                                                                ['Unnamed: 5'][2:8] != 0].to_list()
                saldos_dtc = resultado_rv[2]['Unnamed: 9'][2:8][resultado_rv[2]
                                                                ['Unnamed: 9'][2:8] != 0].to_list()
                ...
                saldo2_gasoa.config(text=saldos_dtc[0])
                saldo2_s500.config(text=saldos_dtc[1])
                saldo2_s10.config(text=saldos_dtc[2])
        ...
        if divisao_modal == '':
            text_gasoa3.insert(
                END, resultado_rondo[1][resultado_rondo[1].columns[0]][0])
            text_s500_3.insert(
                END, resultado_rondo[1][resultado_rondo[1].columns[0]][1])
            text_s10_3.insert(
                END, resultado_rondo[1][resultado_rondo[1].columns[0]][2])
        else:
            text_gasoa4.insert(
                END, str(round(divisao_modal['fe_GASOA'], 3)))
            text_s500_4.insert(
                END, str(round(divisao_modal['fe_DSL500'], 3)))
            text_s10_4.insert(
                END, str(round(divisao_modal['fe_DSL10'], 3)))

            text_gasoa3.insert(END, str(round(divisao_modal['ro_GASOA'], 3)))
            text_s500_3.insert(END, str(round(divisao_modal['ro_DSL500'], 3)))
            text_s10_3.insert(END, str(round(divisao_modal['ro_DSL10'], 3)))
...
janela.minsize(width=270, height=400)
